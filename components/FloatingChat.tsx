...
export default function FloatingChat() {
  ...
  const handleSend = async (forcedText?: string | any) => {
    ...
    setLoading(true);
    try {
      const res = await api.get("/ZChatbotQuerySet", {
        params: {
  ...
  return (
    <>
      ...
      {isOpen && (
        <Animated.View style={[styles.chatWindow, shadow.card, animatedWindowStyle, { bottom: insets.bottom + 90 }]}>
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
            ...
            <LinearGradient colors={gradients.brand} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
              ...
              <TouchableOpacity onPress={() => setIsOpen(false)} style={styles.closeBtn} activeOpacity={0.8}>
