...
useFocusEffect(
  useCallback(() => {
    ...
    // If we DO have data, load silently in the background.
    const hasData = materials.length > 0;
    loadDashboard(!hasData);
  }, [user?.Email]) // Removed 'materials' from dependency to prevent infinite loops
);
...
console.log("Dashboard SAP Response:", data);
  ...
  if (results && results.length > 0) {
    ...
    setLoading(false);
    setRefreshing(false); // Handles Pull-to-Refresh
  }
};
